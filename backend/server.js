...
const app=express(), upload=multer({storage:multer.memoryStorage(),limits:{fileSize:8*1024*1024}}); app.use(cors()); app.use((_,res,next)=>{res.set('Cache-Control','no-store');next()});
const client=new speechV2.SpeechClient({apiEndpoint:`${process.env.GOOGLE_SPEECH_REGION||'eu'}-speech.googleapis.com`});
app.get('/',(_,res)=>res.json({ok:true,service:'LOGG Speech',version:'0.3.0'})); app.get('/health',(_,res)=>res.json({ok:true,service:'LOGG Speech',model:'chirp_3',region:process.env.GOOGLE_SPEECH_REGION||'eu',version:'0.3.0'}));
